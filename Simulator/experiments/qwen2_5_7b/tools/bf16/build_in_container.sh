...
QWEN_LLVM_BIN=${TORCHSIM_LLVM_PATH:-/riscv-llvm/bin}
QWEN_LLVM_INCLUDE=$(dirname -- "$QWEN_LLVM_BIN")/include
case "${1:-all}" in
  all)
...
g++ -std=c++17 -O2 -fno-rtti -fPIC -shared -I"$QWEN_LLVM_INCLUDE" \
...
"$QWEN_LLVM_BIN/mlir-opt" --load-pass-plugin="$QWEN_BUILD_ROOT/libPyTorchSimBF16.so" \
